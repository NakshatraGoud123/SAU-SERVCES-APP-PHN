...
@Composable
fun TechCartScreen(navController: NavController, viewModel: TechServicesViewModel) {
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text("Tech Services Cart", fontWeight = FontWeight.Bold) },
                navigationIcon = {
        ...
        if (viewModel.cartItems.isEmpty()) {
            ...
            LazyColumn(
                ...
                items(viewModel.cartItems) { item ->
                    TechCartItemCard(item, viewModel)
                }
            }
        }
    }
...
@Composable
fun TechCartItemCard(item: TechCartItem, viewModel: TechServicesViewModel) {
    Card(
...
@Composable
fun TechCartBottomBar(viewModel: TechServicesViewModel, onProceed: () -> Unit) {
    Surface(
        modifier = Modifier.fillMaxWidth(),
        color = Color.White,
        tonalElevation = 8.dp,
